...
public class SearchConditionBuilderIntegrationTest {
    ...
    void setUp() {
        ...
        conditionBuilders =
                Arrays.asList(
                        new KeywordPhraseConditionBuilder(), // Order(5) - 最高优先级（新实现）
                        new KeywordComplexExpressionBuilder(), // Order(20) - 已废弃，supports()返回false
                        new KeywordMatchAllConditionBuilder(), // Order(25) - 已废弃，supports()返回false
                        new KeywordMatchAnyConditionBuilder(), // Order(25) - 已废弃，supports()返回false
    ...
    @Test
    @DisplayName("单个简单关键字 - 应使用MATCH_PHRASE")
    public void testSingleSimpleKeyword() {
        /**
         * 测试表达式：["error"] 预期处理器：KeywordPhraseConditionBuilder 预期结果：(message MATCH_PHRASE 'error')
         * 说明：简单的单个关键字应该被MATCH_PHRASE处理器处理
         */
    ...
    @Test
    @DisplayName("单个带引号的关键字 - 应使用MATCH_PHRASE")
    public void testSingleQuotedKeyword() {
        /**
         * 测试表达式：["'database error'"] 预期处理器：KeywordPhraseConditionBuilder 预期结果：(message MATCH_PHRASE
         * 'database error') 说明：带引号的单个关键字也应该被MATCH_PHRASE处理器处理
         */
        LogSearchDTO dto = new LogSearchDTO();
        dto.setKeywords(Collections.singletonList("'database error'"));

        String result = searchConditionManager.buildSearchConditions(dto);
        assertEquals("(message MATCH_PHRASE 'database error')", result);
    }

    @Test
    @DisplayName("多个简单关键字 - 应使用MATCH_PHRASE并用AND连接")
    public void testMultipleSimpleKeywords() {
        /**
         * 测试表达式：["error", "timeout", "critical"] 预期处理器：KeywordPhraseConditionBuilder 预期结果：(message
         * MATCH_PHRASE 'error' AND message MATCH_PHRASE 'timeout' AND message MATCH_PHRASE
         * 'critical') 说明：多个简单关键字应该分别生成MATCH_PHRASE条件，然后用AND连接，整体用括号包装
         */
        LogSearchDTO dto = new LogSearchDTO();
        dto.setKeywords(Arrays.asList("error", "timeout", "critical"));

        String result = searchConditionManager.buildSearchConditions(dto);
        assertTrue(result.contains("message MATCH_PHRASE 'error'"));
        assertTrue(result.contains("message MATCH_PHRASE 'timeout'"));
        assertTrue(result.contains("message MATCH_PHRASE 'critical'"));
    ...
    @Test
    @DisplayName("简单OR表达式 - 应使用MATCH_PHRASE")
    public void testSimpleOrExpression() {
        /**
         * 测试表达式：["'error' || 'warning'"] 预期处理器：KeywordPhraseConditionBuilder 预期结果：(message
         * MATCH_PHRASE 'error' OR message MATCH_PHRASE 'warning')
         * 说明：简单的OR表达式应该被MATCH_PHRASE处理器处理，每个关键字独立
         */
        LogSearchDTO dto = new LogSearchDTO();
        dto.setKeywords(Collections.singletonList("'error' || 'warning'"));

        String result = searchConditionManager.buildSearchConditions(dto);
        assertEquals("(message MATCH_PHRASE 'error' OR message MATCH_PHRASE 'warning')", result);
    }

    @Test
    @DisplayName("多个词的OR表达式 - 应使用MATCH_PHRASE")
    public void testMultipleWordsOrExpression() {
        /**
         * 测试表达式：["'database error' || 'connection timeout' || 'service failure'"]
         * 预期处理器：KeywordPhraseConditionBuilder 预期结果：(message MATCH_PHRASE 'database error' OR
         * message MATCH_PHRASE 'connection timeout' OR message MATCH_PHRASE 'service failure')
         * 说明：多个词组成的OR表达式应该被正确处理
         */
        LogSearchDTO dto = new LogSearchDTO();
        dto.setKeywords(
        ...
        String result = searchConditionManager.buildSearchConditions(dto);
        assertTrue(result.contains("message MATCH_PHRASE 'database error'"));
        assertTrue(result.contains("message MATCH_PHRASE 'connection timeout'"));
        assertTrue(result.contains("message MATCH_PHRASE 'service failure'"));
        assertTrue(result.contains(" OR "));
        assertTrue(result.startsWith("(") && result.endsWith(")"));
    }

    @Test
    @DisplayName("无引号的OR表达式 - 应使用MATCH_PHRASE")
    public void testUnquotedOrExpression() {
        /**
         * 测试表达式：["error || warning || critical"] 预期处理器：KeywordPhraseConditionBuilder 预期结果：(message
         * MATCH_PHRASE 'error' OR message MATCH_PHRASE 'warning' OR message MATCH_PHRASE
         * 'critical') 说明：没有引号的OR表达式也应该被正确处理
         */
        LogSearchDTO dto = new LogSearchDTO();
        ...
        String result = searchConditionManager.buildSearchConditions(dto);
        assertTrue(result.contains("message MATCH_PHRASE 'error'"));
        assertTrue(result.contains("message MATCH_PHRASE 'warning'"));
        assertTrue(result.contains("message MATCH_PHRASE 'critical'"));
        assertTrue(result.contains(" OR "));
        assertTrue(result.startsWith("(") && result.endsWith(")"));
    }
    ...
    @Test
    @DisplayName("简单AND表达式 - 应使用MATCH_PHRASE")
    public void testSimpleAndExpression() {
        /**
         * 测试表达式：["'error' && 'critical'"] 预期处理器：KeywordPhraseConditionBuilder 预期结果：(message
         * MATCH_PHRASE 'error' AND message MATCH_PHRASE 'critical')
         * 说明：简单的AND表达式应该被MATCH_PHRASE处理器处理
         */
        LogSearchDTO dto = new LogSearchDTO();
    ...
    @Test
    @DisplayName("多个词的AND表达式 - 应使用MATCH_PHRASE")
    public void testMultipleWordsAndExpression() {
        /**
         * 测试表达式：["'database' && 'connection' && 'timeout'"] 预期处理器：KeywordPhraseConditionBuilder
         * 预期结果：(message MATCH_PHRASE 'database' AND message MATCH_PHRASE 'connection' AND message
         * MATCH_PHRASE 'timeout') 说明：多个词的AND表达式应该每个词独立处理
         */
        LogSearchDTO dto = new LogSearchDTO();
        dto.setKeywords(Collections.singletonList("'database' && 'connection' && 'timeout'"));

        String result = searchConditionManager.buildSearchConditions(dto);
        assertTrue(result.contains("message MATCH_PHRASE 'database'"));
        assertTrue(result.contains("message MATCH_PHRASE 'connection'"));
        assertTrue(result.contains("message MATCH_PHRASE 'timeout'"));
        assertTrue(result.contains(" AND "));
        assertTrue(result.startsWith("(") && result.endsWith(")"));
    }
    ...
    @Test
    @DisplayName("无引号的AND表达式 - 应使用MATCH_PHRASE")
    public void testUnquotedAndExpression() {
        /**
         * 测试表达式：["database && connection && failed"] 预期处理器：KeywordPhraseConditionBuilder
         * 预期结果：(message MATCH_PHRASE 'database' AND message MATCH_PHRASE 'connection' AND message
         * MATCH_PHRASE 'failed') 说明：没有引号的AND表达式也应该被正确处理
         */
        LogSearchDTO dto = new LogSearchDTO();
        ...
        assertTrue(result.contains("message MATCH_PHRASE 'database'"));
        assertTrue(result.contains("message MATCH_PHRASE 'connection'"));
        assertTrue(result.contains("message MATCH_PHRASE 'failed'"));
        assertTrue(result.contains(" AND "));
        assertTrue(result.startsWith("(") && result.endsWith(")"));
    }
    ...
    @Test
    @DisplayName("简单括号表达式 - 应使用MATCH_PHRASE处理器")
    public void testSimpleParenthesesExpression() {
        /**
         * 测试表达式：["('error' || 'warning') && 'critical'"] 预期处理器：KeywordPhraseConditionBuilder
         * 预期结果：((message MATCH_PHRASE 'error' OR message MATCH_PHRASE 'warning') AND message
         * MATCH_PHRASE 'critical') 说明：包含括号的表达式也会被新的MATCH_PHRASE处理器处理
         */
        LogSearchDTO dto = new LogSearchDTO();
        dto.setKeywords(Collections.singletonList("('error' || 'warning') && 'critical'"));
        ...
        // 新实现会使用MATCH_PHRASE而不是旧的MATCH_ANY/MATCH_ALL
        assertTrue(result.contains("message MATCH_PHRASE"));
    ...
    @Test
    @DisplayName("复杂嵌套表达式 - 应使用MATCH_PHRASE处理器")
    public void testComplexNestedExpression() {
        /**
         * 测试表达式：["('database' || 'service') && ('error' || 'timeout')"]
         * 预期处理器：KeywordPhraseConditionBuilder 预期结果：使用MATCH_PHRASE处理每个关键字
         * 说明：复杂的嵌套表达式也会被新的MATCH_PHRASE处理器正确解析
         */
        LogSearchDTO dto = new LogSearchDTO();
        dto.setKeywords(
    ...
    public void testMixedKeywordAndWhereSqlConditions() {
         ...
         * 'api'"] 预期处理器：KeywordPhraseConditionBuilder + WhereSqlConditionBuilder 预期结果：(message
         * MATCH_PHRASE 'error' AND message MATCH_PHRASE 'timeout') AND (level = 'ERROR') AND
         * (service_name = 'api') 说明：关键字条件和WHERE SQL条件应该都被处理，并用AND连接
         */
        LogSearchDTO dto = new LogSearchDTO();
    ...
    public void testComplexMixedConditions() {
         ...
         * > 5000"] 预期结果：所有关键字都使用MATCH_PHRASE处理，加上WHERE条件
         * 说明：新的KeywordPhraseConditionBuilder处理所有关键字表达式
         */
        LogSearchDTO dto = new LogSearchDTO();
        dto.setKeywords(
                Arrays.asList(
                        "error", // 简单关键字 -> MATCH_PHRASE
                        "'warning' || 'critical'", // OR表达式 -> MATCH_PHRASE
                        "'database' && 'timeout'", // AND表达式 -> MATCH_PHRASE
                        "('service' || 'api') && 'failed'" // 复杂表达式 -> MATCH_PHRASE
                        ));
        dto.setWhereSqls(Arrays.asList("level IN ('ERROR', 'WARN')", "duration > 5000"));

        String result = searchConditionManager.buildSearchConditions(dto);

        // 验证包含MATCH_PHRASE条件（新实现）
        assertTrue(result.contains("message MATCH_PHRASE"));
        assertTrue(result.contains("'error'"));
        assertTrue(result.contains("'warning'"));
        assertTrue(result.contains("'critical'"));
    ...
    @Test
    @DisplayName("条件构建器优先级测试 - KeywordPhraseConditionBuilder优先级最高")
    public void testBuilderPriorityComplexFirst() {
        /**
         * 测试表达式：keywords = ["('error' || 'warning')"] 预期行为：新的KeywordPhraseConditionBuilder有最高优先级
         * 预期处理器：KeywordPhraseConditionBuilder 说明：验证@Order(5)的KeywordPhraseConditionBuilder确实有最高优先级
         */
        LogSearchDTO dto = new LogSearchDTO();
        dto.setKeywords(Collections.singletonList("('error' || 'warning')"));
    ...
    @Test
    @DisplayName("条件构建器优先级测试 - 所有表达式都使用MATCH_PHRASE")
    public void testBuilderPriorityMatchAllVsMatchAny() {
        /**
         * 测试目标：验证新的KeywordPhraseConditionBuilder处理所有关键字表达式
         * 注意：现在所有表达式都由KeywordPhraseConditionBuilder(@Order(5))处理
         */
        ...
        assertTrue(result1.contains("MATCH_PHRASE"));
        assertTrue(result1.contains("'error'"));
        assertTrue(result1.contains("'timeout'"));
        assertTrue(result1.contains(" AND "));
        ...
        assertTrue(result2.contains("MATCH_PHRASE"));
        assertTrue(result2.contains("'error'"));
        assertTrue(result2.contains("'timeout'"));
