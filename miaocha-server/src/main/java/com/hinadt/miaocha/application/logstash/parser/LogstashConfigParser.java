package com.hinadt.miaocha.application.logstash.parser;
...
/** Logstash配置解析器 用于解析Logstash配置文件，提取关键信息 */
@Component
public class LogstashConfigParser {
    ...
    private static final Pattern DORIS_TABLE_PATTERN =
            Pattern.compile(
                    "doris\\s*\\{[\\s\\S]*?table\\s*=>\\s*[\"']([^\"']+)[\"']", Pattern.DOTALL);

    // 匹配Kafka输入部分的正则表达式
    private static final Pattern KAFKA_INPUT_PATTERN =
            Pattern.compile("input\\s*\\{\\s*kafka\\s*\\{([^}]+)\\}\\s*\\}", Pattern.DOTALL);
        ...
        try {
            ...
            if (!kafkaInputMatcher.find()) {
                return ValidationResult.invalid(
                        ErrorCode.LOGSTASH_CONFIG_KAFKA_MISSING, "未找到Kafka输入配置");
            }
            ...
            if (!bootstrapServersMatcher.find()) {
                return ValidationResult.invalid(
                        ErrorCode.LOGSTASH_CONFIG_KAFKA_MISSING, "未找到Kafka bootstrap_servers配置");
            }

            String bootstrapServers = bootstrapServersMatcher.group(1);
            if (!StringUtils.hasText(bootstrapServers)) {
                return ValidationResult.invalid(
                        ErrorCode.LOGSTASH_CONFIG_KAFKA_MISSING, "Kafka bootstrap_servers不能为空");
            }
            ...
            return ValidationResult.invalid(
                    ErrorCode.LOGSTASH_CONFIG_INVALID, "验证Kafka配置时发生错误: " + e.getMessage());
        }
    }
    ...
    public ValidationResult validateDorisOutput(String configContent) {
        ...
        try {
            ...
            if (!configContent.contains("output")) {
                return ValidationResult.invalid(
                        ErrorCode.LOGSTASH_CONFIG_DORIS_MISSING, "未找到output配置");
            }

            // 检查是否有doris输出
            if (!configContent.contains("doris")) {
                return ValidationResult.invalid(
                        ErrorCode.LOGSTASH_CONFIG_DORIS_MISSING, "未找到Doris输出配置");
            }
            ...
            if (!tableNameMatcher.find()) {
                return ValidationResult.invalid(
                        ErrorCode.LOGSTASH_CONFIG_TABLE_MISSING, "未找到Doris表名配置");
            }

            String tableName = tableNameMatcher.group(1);
            if (!StringUtils.hasText(tableName)) {
                return ValidationResult.invalid(
                        ErrorCode.LOGSTASH_CONFIG_TABLE_MISSING, "Doris表名不能为空");
            }
            ...
            return ValidationResult.invalid(
                    ErrorCode.LOGSTASH_CONFIG_INVALID, "验证Doris输出配置时发生错误: " + e.getMessage());
        }
    }
