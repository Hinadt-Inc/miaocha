package com.hinadt.miaocha.domain.validator;

import jakarta.validation.ConstraintValidator;
import jakarta.validation.ConstraintValidatorContext;
import java.util.regex.Pattern;

/** 排序字段验证器 用于验证排序字段名格式，提供详细的错误消息 */
public class SortFieldValidator implements ConstraintValidator<ValidSortField, String> {

    // 匹配 VARIANT 类型字段子字段语法：包含 . 或 [ 或 ' 或 " 的字段
    private static final Pattern VARIANT_FIELD_PATTERN = Pattern.compile(".*[.\\['\"].*");

    // 匹配合法的字段名格式：字母或下划线开头，后面可以跟字母、数字、下划线
    private static final Pattern VALID_FIELD_PATTERN = Pattern.compile("^[a-zA-Z_][a-zA-Z0-9_]*$");

    @Override
    public void initialize(ValidSortField constraintAnnotation) {
        // 初始化方法，这里不需要特殊处理
    }

    @Override
    public boolean isValid(String value, ConstraintValidatorContext context) {
        if (value == null || value.trim().isEmpty()) {
            return true; // null 或空值由其他验证器处理
        }

        // 禁用默认错误消息
        context.disableDefaultConstraintViolation();

        // 检查是否包含 VARIANT 类型字段子字段语法
        if (VARIANT_FIELD_PATTERN.matcher(value).matches()) {
            context.buildConstraintViolationWithTemplate("排序字段不支持 VARIANT 类型字段子字段，只允许普通字段")
                    .addConstraintViolation();
            return false;
        }

        // 检查是否是合法的字段名格式
        if (!VALID_FIELD_PATTERN.matcher(value).matches()) {
            context.buildConstraintViolationWithTemplate("排序字段名格式不正确，必须以字母或下划线开头，只能包含字母、数字、下划线")
                    .addConstraintViolation();
            return false;
        }

        return true;
    }
}
