...
public interface LogstashTaskMachineStepMapper {
         ...
         * @param status 重置后的状态
         */
        @Update("UPDATE logstash_task_machine_step SET status = #{status}, start_time = NULL, end_time = NULL, update_time = NOW() "
                        +
         ...
         * @param taskId 任务ID
         */
        @Update("UPDATE logstash_task_machine_step SET error_message = NULL, update_time = NOW() " +
                        "WHERE task_id = #{taskId}")
         ...
         * @param taskId 任务ID
         */
        @Delete("DELETE FROM logstash_task_machine_step WHERE task_id = #{taskId}")
