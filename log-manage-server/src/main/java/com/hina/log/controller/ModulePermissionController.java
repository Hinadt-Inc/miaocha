...
public class ModulePermissionController {
     ...
     * @return 创建的权限
     */
    @PostMapping("/user/{userId}/datasource/{datasourceId}/grant")
    ...
    public ApiResponse<UserModulePermissionDTO> grantModulePermission(
        ...
        return ApiResponse.success(modulePermissionService.grantModulePermission(userId, datasourceId, module));
    }
     ...
     * @return 结果
     */
    @DeleteMapping("/user/{userId}/datasource/{datasourceId}/revoke")
    @Operation(summary = "撤销模块权限", description = "撤销用户对指定模块的访问权限")
    public ApiResponse<Void> revokeModulePermission(
            @Parameter(description = "用户ID", required = true) @PathVariable("userId") Long userId,
            @Parameter(description = "数据源ID", required = true) @PathVariable("datasourceId") Long datasourceId,
            @Parameter(description = "模块名称", required = true) @RequestParam("module") String module) {
        modulePermissionService.revokeModulePermission(userId, datasourceId, module);
     ...
     * @return 是否有权限
     */
    @GetMapping("/check/datasource/{datasourceId}")
    @Operation(summary = "检查模块权限", description = "检查当前用户是否有指定模块的访问权限")
    public ApiResponse<Boolean> checkModulePermission(
            @CurrentUser UserDTO user,
            @Parameter(description = "数据源ID", required = true) @PathVariable("datasourceId") Long datasourceId,
            @Parameter(description = "模块名称", required = true) @RequestParam("module") String module) {
        boolean hasPermission = modulePermissionService.hasModulePermission(user.getId(), datasourceId, module);
    ...
    @Operation(summary = "获取用户模块权限", description = "获取用户在指定数据源的所有模块权限")
    public ApiResponse<List<UserModulePermissionDTO>> getUserModulePermissions(
            @Parameter(description = "用户ID", required = true) @PathVariable("userId") Long userId,
            @Parameter(description = "数据源ID", required = true) @PathVariable("datasourceId") Long datasourceId) {
        return ApiResponse.success(modulePermissionService.getUserModulePermissions(userId, datasourceId));
    }
