...
public class LogSearchController {
    ...
    @Operation(summary = "执行字段分布查询", description = "仅查询指定字段的TOP5分布数据，使用Doris TOPN函数，字段列表由dto中的fields指定")
    public ApiResponse<LogFieldDistributionResultDTO> searchFieldDistributions(
            @CurrentUser UserDTO user,
            @Parameter(description = "日志检索请求", required = true) @Valid @RequestBody LogSearchDTO dto) {
        LogFieldDistributionResultDTO result = logSearchService.searchFieldDistributions(user.getId(), dto);
        return ApiResponse.success(result);
    }
