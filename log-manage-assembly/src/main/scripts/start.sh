...
# 检查应用是否已经运行
if show_status "log-manage-server"; then
...
while [ $current_time -lt $end_time ]; do
    ...
    # 检查日志中的成功标志
    if grep -q "Started LogManageSystemApplication" "$LOG_DIR/startup.log" 2>/dev/null; then
        echo -e "\r[ ✅ ] 应用启动成功!      "
        break
    fi
...
if ! grep -q "Started LogManageSystemApplication" "$LOG_DIR/startup.log" 2>/dev/null; then
    ...
    # 获取启动时间
    if grep -q "Started LogManageSystemApplication in" "$LOG_DIR/startup.log"; then
        startup_time=$(grep "Started LogManageSystemApplication in" "$LOG_DIR/startup.log" | sed -E 's/.*in ([0-9.]+) seconds.*/\1/')
