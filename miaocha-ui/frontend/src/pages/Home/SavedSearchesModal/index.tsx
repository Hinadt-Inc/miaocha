...
const SavedSearchesModal: React.FC<SavedSearchesModalProps> = ({
  ...
  // 获取缓存的搜索条件
  const { run: fetchCachedSearches } = useRequest(
    logsApi.fetchCachedSearchConditions,
    {
      manual: true,
      onSuccess: (data) => {
  ...
  // 删除缓存搜索条件
  const { run: deleteCachedSearches, loading: deleteLoading } = useRequest(
    logsApi.deleteCachedSearchConditions,
    {
      manual: true,
      onSuccess: () => {
  ...
  useEffect(() => {
    if (!searchKeyword) {
      ...
      const filteredCached = cachedSearches.filter(search =>
  ...
  const handleDeleteSelected = () => {
    ...
    // 按照缓存组分组删除
    const groupedByCache = selectedKeys.reduce((acc, key) => {
  ...
  const handleCheckboxChange = (cacheKey: string) => (e: any) => {
    ...
    checked ? handleSelect(cacheKey) : handleUnselect(cacheKey);
  };

  // 处理全选框变化
  const handleSelectAllChange = (e: any) => {
    const checked = e.target.checked;
    checked ? handleSelectAllItems() : handleUnselectAll();
  };
