...
const SaveSearchModal: React.FC<SaveSearchModalProps> = ({
  ...
  // 保存搜索条件
  const { loading, run: saveSearchCondition } = useRequest(
    logsApi.saveSearchConditionWithCache,
    {
      manual: true,
      onSuccess: (response) => {
        ...
        <div className={styles.previewContent}>
          ...
          {searchParams?.sortConfig?.length > 0 && (
            <div>
              <strong>排序:</strong>
              <span>
                {searchParams.sortConfig.map((item: any) => 
                  `${item.fieldName} (${item.direction === 'ASC' ? '升序' : '降序'})`
                ).join(', ')}
              </span>
            </div>
          )}
