...
export default function HomePage() {
  ...
  const [tableKey, setTableKey] = useState(0) // 用于强制重新渲染表格
  ...
  const getTableColumns = () => {
    return selectedFields.map(field => ({
      ...
      className: 'table-column-with-animation', // 更新为新的类名
    }))
  }
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 16 }}>
          <Space size="middle">
            ...
            <RangePicker 
              ...
              value={timeRange ? [
                timeRange[0] ? dayjs(`${timeRange[0]} 00:00:00`) : null,
                timeRange[1] ? dayjs(`${timeRange[1]} 23:59:59`) : null
              ] : null}
              onChange={(dates) => {
                  ...
                  {availableFields.map(field => (
                    <div 
                      ...
                      style={{ 
                        ...
                        marginBottom: 4,
                        transition: 'all 0.3s ease' // 直接添加过渡效果
                      }}
                      onClick={() => toggleFieldSelection(field.name)}
                      className="field-selection-item" // 添加类名用于样式选择器
                    >
                      <Space>
                      ...
                      {selectedFields.includes(field.name) && (
                        <EyeOutlined 
                          style={{ 
                            color: '#1890ff',
                            animation: 'fadeIn 0.3s ease' // 直接添加内联动画
                          }} 
                        />
                      )}
                    </div>
